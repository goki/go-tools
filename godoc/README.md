...
executable lives at github.com/goki/go-tools/cmd/godoc.
...
```
godoc -templates=$GOPATH/src/github.com/goki/go-tools/godoc/static -http=:6060
```
...
2) Run `go generate github.com/goki/go-tools/godoc/static` so `static/static.go` picks
up the change.

3) Run `go install github.com/goki/go-tools/cmd/godoc` so the compiled `godoc` binary
picks up the change.
