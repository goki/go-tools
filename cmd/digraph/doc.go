...
why the gopls application depends on the cmp package:
	...
	$ go list -f '{{.ImportPath}} {{join .Imports " "}}' -deps github.com/goki/go-tools/gopls |
		digraph somepath github.com/goki/go-tools/gopls github.com/google/go-cmp/cmp

Show which packages in x/tools depend, perhaps indirectly, on the callgraph package:

	$ go list -f '{{.ImportPath}} {{join .Imports " "}}' -deps github.com/goki/go-tools/... |
		digraph reverse github.com/goki/go-tools/go/callgraph
